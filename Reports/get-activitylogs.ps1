[CmdletBinding()]
    param (
        ...
        [string]$AssetName,
        [Parameter( Mandatory=$true)]
        [ValidateSet("FULL","LOG","DIFFERENTAL")]
        [string]$BackupType,
        [Parameter( Mandatory=$true)]
        [int]$BackupWindow
    )
...
# VARS
$Server = "10.239.100.131"
$PageSize = 100
$Poll = 20
$Date = (get-date).AddHours(-$BackupWindow)
$Path = "C:\Reports\customers\dell\output"
...
$Filters = @(
    ...
    "and subcategory eq `"$($BackupType)`"",
    "and result.status eq `"FAILED`"",
    "and state eq `"COMPLETED`"",
    "and host.name eq `"$($HostName)`"",
    "and asset.name eq `"$($AssetName)`""
    "and startTime gt `"$($Date.ToString('o'))`""
)
# GET ASSETS BASED ON FILTERS
Write-Host "[PowerProtect Data Manager]: Gathering failed acitivities for: $($HostName)\$($AssetName)" `
-ForegroundColor Green
$Activities = get-dmactivities -Filters $Filters -PageSize $PageSize

if($Activities.length -gt 0) {
    Write-Host "[PowerProtect Data Manager]: $($Activities.length) failed activities found for: $($HostName)\$($AssetName)`n" `
    -ForegroundColor Yellow
} else {
    Write-Host "[PowerProtect Data Manager]: No failed activities found for: $($HostName)\$($AssetName)`n" `
    -ForegroundColor Yellow
}

$Activities | foreach-object {
    Write-Host "[$($Server)]: Exporting logs for $($HostName)\$($_.asset.name)"
    # GENERATE THE EXPORT
