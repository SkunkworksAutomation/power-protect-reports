...
$Servers | ForEach-Object { 
    foreach($Retry in $Retires) {
        try {
            ...
            Write-Host "[PowerProtect Data Manager]: Getting activity report with a startTime > $($Date.ToString('yyyy-MM-ddThh:mm:ss.fffZ'))" `
...
$Excel.cells.item($HeaderRow,15) = "assetSize"
$Excel.cells.item($HeaderRow,16) = "bytesTransferred"
$Excel.cells.item($HeaderRow,17) = "storageTarget"
$Excel.cells.item($HeaderRow,18) = "replicationTarget"
...
for($i=0;$i -lt $Activities.length; $i++) {
    ...
    $Excel.cells.item($RowOffSet,18) = $Activities[$i].replicationTarget
