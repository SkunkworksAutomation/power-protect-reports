...
$Excel.cells.item($HeaderRow,4) = "username"
$Excel.cells.item($HeaderRow,5) = "type"
$Excel.cells.item($HeaderRow,6) = "method"
$Excel.cells.item($HeaderRow,7) = "internal"
$Excel.cells.item($HeaderRow,8) = "consumersCount"
$Excel.cells.item($HeaderRow,9) = "createdByUser"
...
for($i=0;$i -lt $Report.length; $i++) {
    ...
    $Excel.cells.item($RowOffSet,4) = $Report[$i].username
    $Excel.cells.item($RowOffSet,5) = $Report[$i].type
    $Excel.cells.item($RowOffSet,6) = $Report[$i].method
    $Excel.cells.item($RowOffSet,7) = $Report[$i].internal
    $Excel.cells.item($RowOffSet,8) = $Report[$i].consumersCount
    $Excel.cells.item($RowOffSet,9) = $Report[$i].createdByUser
